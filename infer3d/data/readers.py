...
from infer3d.utils.graphics import focal2fov, fov2focal
import numpy as np
...
def readCamerasFromTxt(rgb_paths, pose_paths, idxs, no_imgs=False):
    cam_infos = []
    ...
    for idx in idxs:
        ...
        if no_imgs:
            img_size = 128 
            fovy = focal2fov(fov2focal(fovx, img_size), img_size)
            FovY = fovy 
            FovX = fovx
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=None,
                            image_path=None, image_name=Path(cam_name).stem, width=img_size, height=img_size))
            continue
