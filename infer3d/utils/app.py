...
def set_black_background(image):
    image = np.array(image).astype(np.float32) / 255.0
    if image.shape[2] == 4:
        mask = image[:, :, 3:4]  # alpha channel
    else:
        mask = np.ones_like(image[:, :, :1])  # fully opaque if no alpha
    image = image[:, :, :3] * mask + (1 - mask) * np.zeros_like(image[:, :, :3])  # black background
...
def set_white_background(image):
    ...
    if image.shape[2] == 4:
        ...
        mask = np.ones_like(image[:, :, :1])  # fully opaque if no alpha
    image = image[:, :, :3] * mask + (1 - mask)  # white background
    image = Image.fromarray((image * 255.0).astype(np.uint8))
    return image
...
def resize_foreground_w_size_output(image, ratio, output_size=128):
    image = np.array(image)
    assert image.shape[-1] == 4  # Ensure image has an alpha channel
    
    # Extract alpha channel
    alpha = image[..., 3]
    nonzero_coords = np.where(alpha > 0)
    ...
    # Compute new foreground size based on the given ratio
    fg_size = int(output_size * ratio)

    # Resize the image keeping the foreground ratio
    new_image = Image.fromarray(new_image).resize((fg_size, fg_size), Image.LANCZOS)

    # Create a blank white background of 128x128
    final_image = Image.new("RGBA", (output_size, output_size), (255, 255, 255, 0))

    # Paste resized foreground at the center
    paste_x = (output_size - fg_size) // 2
    paste_y = (output_size - fg_size) // 2
    final_image.paste(new_image, (paste_x, paste_y), new_image)

    return final_image

def generate_mask_from_alpha(image):
    """Generate mask tensor from an RGBA image."""
    image = np.array(image)
    mask = image[..., 3]  # Extract alpha channel
    mask = np.where(mask < 255, 0, 1)  # Convert to binary mask
    return torch.from_numpy(mask).unsqueeze(0)  # Add channel dimension



def resize_foreground(image, ratio, background_color="white"):
    image = np.array(image)
    assert image.shape[-1] == 4
    alpha = np.where(image[..., 3] > 0)
    # modify so that cropping doesn't change the world center
...
def get_source_camera_v2w_rmo_and_quats(num_imgs_in_loop=200, radius=2.0, center=np.array([0, 0, 0])):
    source_camera = get_loop_cameras(num_imgs_in_loop=num_imgs_in_loop, radius = radius, center=center)[0]
    source_camera = torch.from_numpy(source_camera).to(dtype=torch.float32).transpose(0, 1).unsqueeze(0)
...
def get_target_cameras(num_imgs_in_loop=200, projection_matrix = None, radius=2.0, center=np.array([0, 0, 0]), cfg = None):
    """
    Returns camera parameters for rendering a loop around the object:
      ...
      camera_centers
    """
    if projection_matrix is None and cfg is None: 
        projection_matrix = getProjectionMatrix(
            znear=0.8, zfar=3.2,
            fovX=49.134342641202636 * 2 * np.pi / 360, 
            fovY=49.134342641202636 * 2 * np.pi / 360).transpose(0,1).to(dtype=torch.float32)
    elif cfg is not None: 
        projection_matrix = getProjectionMatrix(
            znear=cfg.data.znear, zfar=cfg.data.zfar,
            fovX=cfg.data.fov * 2 * np.pi / 360, 
            fovY=cfg.data.fov * 2 * np.pi / 360).transpose(0,1).to(dtype=torch.float32)
    ...
    for loop_camera_c2w_cmo in target_cameras:
        view_world_transform = torch.from_numpy(loop_camera_c2w_cmo).to(dtype=torch.float32).transpose(0, 1)
        world_view_transform = torch.from_numpy(loop_camera_c2w_cmo).to(dtype=torch.float32).inverse().transpose(0, 1)
