...
from infer3d.utils.graphics import fov2focal
...
class GaussianSplatPredictor(nn.Module):
    ...
    def init_ray_dirs(self):
        ...
        if self.cfg.data.category not in ["hydrants", "teddybears", "hydrants_multiview", "teddybears_multiview", "motorcycles", "plants", "vases"]:
    ...
    def get_pos_from_network_output(self, depth_network, offset, focals_pixels, const_offset=None):
        ...
        if self.cfg.data.category in ["hydrants", "teddybears", "hydrants_multiview", "teddybears_multiview", "motorcycles", "plants", "vases"]:
    ...
    def forward(self, x, 
        ...
        if self.cfg.data.origin_distances and x.shape[1] > 3:
            const_offset = x[:, 3:, ...]
            x = x[:, :3, ...]
        else:
            const_offset = None
