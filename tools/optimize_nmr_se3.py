...
import csv 
# (legacy stylegan2-pytorch path removed)
...
from infer3d.data.shapenet_nmr import ShapenetNMR
import imageio
...
# HACK: Load all SE3 rendered object IDs
def load_se3_rendered_object_ids():
    """Load all object IDs that have SE3 renders"""
    se3_object_ids = set()
    se3_files_dir = f"{ROOT}/rendering_scripts/vincent-renderer"
    
    # Map of category names to file names
    category_file_map = {
        "airplane": "rendered_se3_nmr_airplane.txt",
        "bench": "rendered_se3_nmr_bench.txt",
        "cabinet": "rendered_se3_nmr_cabinet.txt",
        "car": "rendered_se3_nmr_car.txt",
        "chair": "rendered_se3_nmr_chair.txt",
        "display": "rendered_se3_nmr_display.txt",
        "lamp": "rendered_se3_nmr_lamp.txt",
        "loudspeaker": "rendered_se3_nmr_loudspeaker.txt",
        "rifle": "rendered_se3_nmr_rifle.txt",
        "sofa": "rendered_se3_nmr_sofa.txt",
        "table": "rendered_se3_nmr_table.txt",
    }
    
    for category, filename in category_file_map.items():
        filepath = os.path.join(se3_files_dir, filename)
        if os.path.exists(filepath):
            with open(filepath, 'r') as f:
                for line in f:
                    ...
                    if obj_id:
                        # check if 
                        test_file_path = f"{_cfg.SHAPENET_NMR_ROOT}/{obj_id}/test/intrinsics.txt"
                        side_top_file_path = f"{_cfg.SHAPENET_NMR_ROOT}/{obj_id}/side-top/intrinsics.txt"
                        if os.path.exists(test_file_path) and os.path.exists(side_top_file_path):
                            se3_object_ids.add(obj_id)
        else:
            print(f"Warning: SE3 rendered file not found: {filepath}")
    
    print(f"Loaded {len(se3_object_ids)} object IDs with SE3 renders")
    return se3_object_ids

SE3_RENDERED_OBJECTS = load_se3_rendered_object_ids()
...
def eval_baseline(ood_data, ood_index, eval_data, cur_focals_pixels, gaussian_predictor, cfg, background, save_video_path=None):
...
def eval_pred(best_input_image, best_rotation_matrix, best_translation_matrix, eval_data, cur_focals_pixels, zgt, background, ood_data, ood_index, gaussian_predictor, cfg, save_video_path=None, zgt_ood=None): 
    """Evaluate our optimized method"""
    ood_data = {k: (v.to(device) if isinstance(v, torch.Tensor) else v) for k, v in ood_data.items()}
    ood_image = ood_data["gt_images"][0, ood_index].to(device)
    assert zgt_ood is not None, "Please provide zgt_ood"    
    pred_splats = gaussian_predictor(
    ...
    transformed_splats = render_with_custom_camera(pred_splats, 
                                                   ...
                                                   zgt, device = device, return_splats=True, translation=best_translation_matrix.to(device), zgt_ood=zgt_ood)
    eval_data_relative = make_data_relative_to(target_data=eval_data, reference_data=ood_data)
...
def load_dataset(dataset_name, val_view, test_idx, train_example_ids, override_example_id, cfg):
    ...
    if dataset_name == "shapenet_nmr": 
        ...
        dataset = ShapenetNMR(cfg, "train", override_example_ids=override_example_ids, override_overall_view="side-top", override_root=_cfg.SHAPENET_NMR_ROOT)
        eval_data = ShapenetNMR(cfg, "test", override_overall_view="test", override_example_ids=override_example_ids, deterministic_test_idxs = [0], override_root=_cfg.SHAPENET_NMR_ROOT)
        
        val_dataset = ShapenetNMR(cfg, "test", override_overall_view=val_view, override_example_ids=override_example_ids, deterministic_test_idxs = [test_idx], override_root=_cfg.SHAPENET_NMR_SE3_ROOT)
        ...
        override_example_ids = [override_example_id] if override_example_id is not None else None
        assert override_example_ids is not None, "Please provide an example id to override"
        dataset = ShapenetNMR(cfg, "train", override_example_ids=override_example_ids, override_overall_view="side-top", particular_class_list=[dataset_name], override_root=_cfg.SHAPENET_NMR_ROOT)
        eval_data = ShapenetNMR(cfg, "test", override_overall_view="test", override_example_ids=override_example_ids, deterministic_test_idxs = [0], particular_class_list=[dataset_name], override_root=_cfg.SHAPENET_NMR_ROOT)
        
        val_dataset = ShapenetNMR(cfg, "test", override_overall_view=val_view, override_example_ids=override_example_ids, deterministic_test_idxs = [test_idx], particular_class_list=[dataset_name], override_root=_cfg.SHAPENET_NMR_SE3_ROOT)
...
def load_models(): 
    ...
    generator_ckpt_path = _cfg.STYLEGAN_CKPTS["shapenet_nmr"]
...
def train(cur_cfg, override_example_id = None, test_idx = 0, log_path = "checkpoints-side-top-se3-last-try", val_view = None, dataset_name = None, prefix = None): 
    ...
    if not cfg.abs.custom_ood_image and not cfg.abs.random_ood_image:
        ...
        w2c = ood_data["world_view_transforms_absolute"][0, ood_image_index].to(device)
        # get ood_zgt from w2c
        ood_zgt = w2c[3, 2]  # shape [3, 3]
        print("ood_zgt", ood_zgt)
        ...
        from infer3d.utils.app import remove_background, resize_foreground, set_white_background, resize_foreground_w_size_output, generate_mask_from_alpha
        ...
        if iteration >= config_steps[step_idx]["steps"]:
            ...
            if settings == "stop":
                ...
                dict_to_log = {
                    ...
                    "best_input_image": wandb.Image(log_best_input_image, caption="Best Input Image"),
                ...
                for idx in range(top_k):
                    ...
                    _, _, cur_scores = eval_pred(eval_input_image, eval_rotation_matrix, eval_translation_matrix, eval_data, ood_focals_pixels, zgt, background, ood_data, ood_image_index, gaussian_predictor, cfg, save_video_path=ours_video_path, zgt_ood=ood_zgt)
                ...
                known_best_renders, known_best_gt_images, known_best_scores = eval_pred(best_input_image, best_rotation_matrix, best_translation_matrix, eval_data, ood_focals_pixels, zgt, background, ood_data, ood_image_index, gaussian_predictor, cfg, save_video_path=ours_best_video_path, zgt_ood=ood_zgt)
...
def main(cfg:  DictConfig): 
    ...
    results_save_path = f"{log_path}/side_top_se3_results_HACK.csv"
    
    # HACK: Filter to only process objects with SE3 renders
    if all([cfg.general.override_example_id, cfg.general.ood_view]): 
        print("using custom override example id")
        # Check if this object has SE3 renders
        if cfg.general.override_example_id not in SE3_RENDERED_OBJECTS:
            print(f"WARNING: Object {cfg.general.override_example_id} does not have SE3 renders. Skipping.")
            return
    ...
    assert cfg.general.data_example_ids_path is not None, \
        "the ShapeNet-NMR runners need general.data_example_ids_path=<splits json>"
    with open(cfg.general.data_example_ids_path, "r") as f: 
        data_example_ids_with_views = json.load(f)
    
    # HACK: Filter to only include objects with SE3 renders
    test_ids_to_use = []
    for item in data_example_ids_with_views:
        override_example_id = item[0]
        if override_example_id in SE3_RENDERED_OBJECTS:
            test_ids_to_use.append(item)
    
    print(f"HACK: Filtered from {len(data_example_ids_with_views)} to {len(test_ids_to_use)} objects with SE3 renders")
    ...
    for override_example_id, view, test_img_idx in cur_example_ids: 
        try: 
            ...
            start_time = time.time()
            val_view = ood_view  # Always use se3 view
