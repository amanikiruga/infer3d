...
def train(cur_cfg, override_example_id = None, test_idx = 0, log_path = "checkpoints-side-top-so3-last-try", val_view = None, dataset_name = None, prefix = None): 
    ...
    while step_idx < len(config_steps):
        ...
        if iteration >= config_steps[step_idx]["steps"]:
            ...
            # ---- ABLATION GATE (only diff vs rebuttal/baselines/ours_nmr_so3.py) ----
            # A loss term whose cfg weight is exactly 0 is an ablated term and must stay 0
            # through ALL stages. Stages 3/4 of config_steps HARDCODE lambda_mse=10/2 and
            # lambda_lpips=2, which would silently re-enable an ablated term (and, for LPIPS,
            # crash because lpips_fn is only built when cfg.abs.lambda_lpips != 0). Re-zero here.
            # (lambda_mse / lambda_lpips are already declared global at the top of train().)
        ...
        for batch_idx in range(num_batches):
            ...
            # ABLATION GATE: the selection ("decision") loss must not rank particles by a
            # term that has been ablated from the objective. Default is l_lpips + 4*l_mse;
            # drop whichever term's cfg weight is 0 so best-particle selection is consistent
            # with what is being optimized. (Both are never 0 simultaneously in our arms.)
            _w_lpips_dec = 1.0 if float(cfg.abs.lambda_lpips) != 0.0 else 0.0
            _w_mse_dec = 4.0 if float(cfg.abs.lambda_mse) != 0.0 else 0.0
            decision_losses = torch.tensor([_w_lpips_dec * l_lpips.item() + _w_mse_dec * l_mse.item()
                                            for l_mse, l_lpips in zip(mse_losses, lpips_loss_values)])
