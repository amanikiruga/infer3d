...
from splatter_image_datasets.co3d import CO3DDataset
from splatter_image_datasets.co3d_fastgs import CO3DFastGSDataset
import imageio
...
def train(cur_cfg, override_example_id = None, test_idx = 0, log_path = "checkpoints-co3d-se3", val_view = None, dataset_name = None, prefix = None, test_img_path = None, run_indist = False): 
    ...
    while step_idx < len(config_steps):
        ...
        if iteration >= config_steps[step_idx]["steps"]:
            ...
            # ---- ABLATION GATE (only logic diff vs the compute_cost source) ----
            # A loss term whose cfg weight is exactly 0 is ablated and must stay 0 through ALL
            # stages. config_steps stage 302 HARDCODES lambda_lpips=2 / lambda_mse=10 (and 732
            # lambda_mse=2), which would silently re-enable an ablated term (and, for LPIPS,
            # crash because lpips_fn is only built when cfg.abs.lambda_lpips != 0). Re-zero the
            # affected globals; the decision/selection loss reads these same globals, so
            # particle selection stays consistent with the optimized objective automatically.
            # (lambda_mse / lambda_lpips / lambda_depth are declared global at the top of train.)
            if float(cfg.abs.lambda_mse) == 0.0:
                lambda_mse = 0
            if float(cfg.abs.lambda_lpips) == 0.0:
                lambda_lpips = 0
            if float(cfg.abs.lambda_depth) == 0.0:
                lambda_depth = 0
            # (w_reg latent prior is applied directly from cfg.abs.w_reg and excluded from the
            #  decision loss, so abs.w_reg=0 alone cleanly ablates it — no gating needed.)
            # -------------------------------------------------------------------
