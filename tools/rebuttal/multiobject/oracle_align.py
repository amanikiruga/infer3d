...
OUT = f"{WT}/rebuttal/multiobject/oracle_align"
...
def main():
    ...
    for scene in sorted(os.listdir(f"{SRC}/stage1")):
        ...
        merged = {k: v.detach() for k, v in merged.items()}
        ...
        # residual similarity on the splats, initialized to IDENTITY so init == the
        # pipeline's own 14.04 render; the optimizer can only improve -> true upper bound.
        ...
        opt = torch.optim.Adam([{"params": [six], "lr": 0.01},
                                {"params": [log_s, t], "lr": 0.005}])
        ...
        with torch.no_grad():
            p0 = render_eval(merged)
            init_psnr = float(-10 * torch.log10(((p0 - g) ** 2).mean() + 1e-12))
        best = init_psnr
        for it in range(400):
            opt.zero_grad()
            sp = transform_splats(merged, torch.exp(log_s), sixd_to_R(six), t)
            pred = render_eval(sp)
            mse = ((pred - g) ** 2).mean()
            mse.backward()
            opt.step()
        ...
        print(f"{scene[:20]}: init {init_psnr:.2f} -> oracle {best:.2f}  (+{best-init_psnr:.2f})")
