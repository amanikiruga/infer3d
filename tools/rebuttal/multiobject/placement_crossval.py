...
OUT = f"{WT}/rebuttal/multiobject/placement_crossval"
...
def main():
    ...
    for scene in sorted(os.listdir(f"{SRC}/stage1")):
        ...
        # CROSS-VALIDATED per-object registration: 2x7=14-DoF similarity is a low-DoF
        # registration (like the ICP alignment used in every Chamfer eval). Fit it on a
        # FIT half of the views, report the HELD-OUT half — if held-out PSNR stays high,
        # the placement is genuine registration, not test-view overfitting.
        all_novel = list(range(1, gt.shape[0]))
        fit_idx = all_novel[0::2]      # even-indexed novel views
        hold_idx = all_novel[1::2]     # held-out
        g_fit, g_hold = gt[fit_idx], gt[hold_idx]
        ...
        def render_at(sp, idxs):
            return torch.stack([render_predicted(sp, wvts[i:i+1], fpts[i:i+1], ccs[i:i+1], bg, cfgr,
                                                 focals_pixels=None)["render"].clamp(0, 1) for i in idxs])

        with torch.no_grad():
            init_hold = float(-10 * torch.log10(((render_at(merged, hold_idx) - g_hold) ** 2).mean() + 1e-12))
        for it in range(400):
            opt.zero_grad()
            pred = render_at(compose_scene(), fit_idx)   # FIT on even views only
            (((pred - g_fit) ** 2).mean()).backward()
            opt.step()
        with torch.no_grad():
            sp = compose_scene()
            hold = float(-10 * torch.log10(((render_at(sp, hold_idx) - g_hold) ** 2).mean() + 1e-12))
            fitp = float(-10 * torch.log10(((render_at(sp, fit_idx) - g_fit) ** 2).mean() + 1e-12))
        res[scene] = {"camera_scale": camera_scale, "init_holdout_psnr": init_hold,
                      "fit_psnr": fitp, "heldout_psnr": hold}
        print(f"{scene[:20]}: init-holdout {init_hold:.2f} -> fit {fitp:.2f} / HELD-OUT {hold:.2f}")

    P = np.array([v["heldout_psnr"] for v in res.values()])
    I = np.array([v["init_holdout_psnr"] for v in res.values()])
    summary = {"n": len(P), "init_holdout_mean": float(I.mean()), "heldout_mean": float(P.mean()),
               "heldout_median": float(np.median(P)), "gain_mean": float((P - I).mean()),
               "ge18": int((P >= 18).sum()), "ge16": int((P >= 16).sum()), "per_scene": res}
    print(json.dumps({k: summary[k] for k in ["n", "init_holdout_mean", "heldout_mean", "heldout_median", "gain_mean", "ge18", "ge16"]}, indent=1))
    json.dump(summary, open(f"{OUT}/crossval_summary.json", "w"), indent=1)
